"""Loads a service's app/ modules directly from its source tree.

Every service's package is named "app", so a plain `import app.model`
would collide once more than one service is loaded in the same process.
Each module gets imported under a unique alias instead (e.g.
"recsvc_app"), with that alias registered as a real package pointing at
the service's app/ directory, so relative imports inside the module
(like `from .kafka_consumer import new_consumer`) resolve normally.

Shared by tests/conftest.py and experiments/ scripts that need to run
the actual production algorithm (not a reimplementation) offline.
"""
...
# version). Nothing offline calls it -- recommendation-service's
