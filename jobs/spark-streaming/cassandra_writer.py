...
def _get_session():
    ...
    # Partitioned by bucket_date alone, so serving-optimizer can look up
    # "which products were active on date X" with a direct partition
    # read instead of scanning every partition in product_demand_by_minute.
    session.execute(
        f"""
        CREATE TABLE IF NOT EXISTS {ACTIVE_PARTITIONS_TABLE} (
            bucket_date date,
            product_id int,
            PRIMARY KEY (bucket_date, product_id)
        )
        """
    )
    ...
    active_partition_stmt = session.prepare(
        f"INSERT INTO {ACTIVE_PARTITIONS_TABLE} (bucket_date, product_id) VALUES (?, ?)"
    )

    _state["session"] = session
    _state["insert_stmt"] = insert_stmt
    _state["active_partition_stmt"] = active_partition_stmt
...
def write_demand_count(product_id: int, window_start, event_count: int) -> None:
    ...
    bucket_date = window_start.date()
    session.execute(insert_stmt, (product_id, bucket_date, window_start, event_count))
    session.execute(_state["active_partition_stmt"], (bucket_date, product_id))
